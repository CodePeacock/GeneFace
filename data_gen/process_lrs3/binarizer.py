...
if __name__ == '__main__':
    ...
    for prefix in prefixs:
        ...
        spk_ids = sorted(
            [
                dir_name.split("/")[-1]
                for dir_name in glob.glob(f"{raw_base_dir}/*")
            ]
        )
        ...
        mp4_names = glob.glob(f"{raw_base_dir}/*/*.mp4")
        cnt = 0
        for i, mp4_name in tqdm.tqdm(enumerate(mp4_names), total=len(mp4_names)):
            ...
            spk_id = lst[-2]
            clip_id = lst[-1][:-4]
            audio_npy_name = os.path.join(raw_base_dir, spk_id, f"{clip_id}_audio.npy")
            hubert_npy_name = os.path.join(raw_base_dir, spk_id, f"{clip_id}_hubert.npy")
            video_npy_name = os.path.join(raw_base_dir, spk_id, f"{clip_id}_coeff_pt.npy")
            if (not os.path.exists(audio_npy_name)) or (not os.path.exists(video_npy_name)):
                print("Skip item for not found.")
                continue
            if (not os.path.exists(hubert_npy_name)):
                print("Skip item for hubert_npy not found.")
                continue
